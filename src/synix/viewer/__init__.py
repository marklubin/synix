...
if TYPE_CHECKING:
    from synix.sdk import Release

logger = logging.getLogger(__name__)


def create_app(release: Release, *, title: str = "Viewer"):
    ...
    state = ViewerState(release, title)
    return _create_app(state)


def serve(release: Release, *, host: str = "127.0.0.1", port: int = 9471, title: str = "Viewer"):
    """Start the viewer dev server."""
    app = create_app(release, title=title)
