...
case = {
    ...
    "steps": [
        ...
        {"name": "note_build", "command": ["synix", "demo", "note",
            "2/10 Building product briefs from catalog + policies..."]},
        {"name": "build", "command": ["synix", "build", "PIPELINE"]},

        # Step 3: Search
        {"name": "note_search", "command": ["synix", "demo", "note",
            "3/10 Searching built artifacts..."]},
        {"name": "search", "command": [
            "synix", "search", "return policy", "--mode", "keyword", "--limit", "3"]},

        # Step 4: Verify
        {"name": "note_verify", "command": ["synix", "demo", "note",
            "4/10 Verifying build integrity..."]},
        ...
        {"name": "note_validate", "command": ["synix", "demo", "note",
            "5/10 Validating for contradictions and PII..."]},
        {"name": "validate_initial", "command": ["synix", "validate", "PIPELINE", "--json"],
         "capture_json": True},

        # Step 6: Fix PII at source — edit the data, not the artifact
        {"name": "note_pii", "command": ["synix", "demo", "note",
            "6/10 Fixing PII at the source..."]},
        ...
        {"name": "note_rebuild", "command": ["synix", "demo", "note",
            "7/10 Rebuilding (source data changed)..."]},
        {"name": "build_mid", "command": ["synix", "build", "PIPELINE"]},

        # Step 8: Re-validate — PII gone, semantic conflicts remain
        {"name": "note_revalidate", "command": ["synix", "demo", "note",
            "8/10 Re-validating (PII should be gone)..."]},
        {"name": "validate_mid", "command": ["synix", "validate", "PIPELINE", "--json"],
         "capture_json": True},

        # Step 9: Fix semantic conflicts (LLM-powered, artifact-level)
        {"name": "note_fix", "command": ["synix", "demo", "note",
            "9/10 Fixing semantic conflicts with LLM..."]},
        {"name": "fix", "command": ["synix", "fix", "PIPELINE"], "stdin": "a\na\na\n"},

        # Step 10: Final validate — should be clean
        {"name": "validate_final", "command": ["synix", "validate", "PIPELINE", "--json"],
         "capture_json": True},
    ],
    "goldens": {
